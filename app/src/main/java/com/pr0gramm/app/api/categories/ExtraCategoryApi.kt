...
interface ExtraCategoryApi {
    @GET("random")
    fun random(@Query("tags") tags: String?,
               @Query("flags") flags: Int): Observable<Api.Feed>

    @GET("controversial")
    fun controversial(@Query("tags") tags: String?,
                      @Query("flags") flags: Int,
                      @Query("older") older: Long?): Observable<Api.Feed>

    @GET("text")
    fun text(@Query("tags") tags: String?,
             @Query("flags") flags: Int,
             @Query("older") older: Long?): Observable<Api.Feed>

    @GET("bestof")
    fun bestof(@Query("tags") tags: String?,
    ...
    @GET("general")
    fun general(@Query("promoted") promoted: Int?,
